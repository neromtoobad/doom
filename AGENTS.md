...
➠ **the Echo demo hardcodes a 5 STRK withdrawal** (`FIVE_STRK` in `WalletAccountV6Tag.tsx`). shielding less than that makes it fail with a misleading *gas* error, not an insufficient-balance one. when a STRK20 action reports a fee problem, suspect a reverting assembly first
➠ **one private tx costs ~3.7 STRK in fees.** budget accordingly: ~20 mainnet txs is ~75 STRK
➠ **Braavos is not privacy-enabled.** it cannot perform STRK20 actions at all. the starter kit hides it from the wallet picker deliberately — do not "fix" that, it connects fine and then fails on the first shield. **Ready is the tested baseline** for the Wallet API route, Xverse is in progress. the docs say to detect capability rather than infer it from the brand name
